...
def papers_table():
    ...
    tot_c = sum(int(r[2].split("/")[0]) for r in rows if "/" in r[2])
    tot_s = sum(int(r[2].split("/")[1]) for r in rows if "/" in r[2])
    out = [MARK_A,
           "",
    ...
    out += ["",
            f"**Claims covered** is how many of the paper's claims that a re-run could settle "
            f"— its `empirical` and `control` claims carrying a reproduction record — the "
            f"script actually produces a result for. Corpus-wide that is "
            f"**{tot_c} of {tot_s}**. The remaining {tot_s - tot_c} carry a status an agent "
            f"reached by reading a deposit rather than by running this code. That is a "
            f"weaker kind of verification, and the number is here because nothing else on "
            f"the site says so.",
            "",
